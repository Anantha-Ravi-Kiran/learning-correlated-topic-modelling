...
if [ $# -ne 8 -a $# -ne 9 ];
  then
    echo "Usage: <folder containing files/corpus> <output folder> <num_topics> <num_comp> <gibbs_itrn> <model> <BETA_SMOOTH> <noise_prop_optional>"
	exit 0
fi
...
do
	...
	if [ $# -ne 9 ]	; then
		python ./topic-modelling/learn_params.py  $OUTPUT_FOLDER/$corpus\_train.scipy $OUTPUT_FOLDER/demo_L2_out.$corpus.$NUM_TOPICS.A  $OUTPUT_FOLDER/demo_L2_out.$corpus.$NUM_TOPICS.topwords $OUTPUT_FOLDER/report $NUM_COMP $NUM_ITRN $corpus $GIBBS_ITRN $MODEL $BETA_SMOOTH $SEED

	else
		python ./topic-modelling/learn_params.py  $OUTPUT_FOLDER/$corpus\_train.scipy $OUTPUT_FOLDER/demo_L2_out.$corpus.$NUM_TOPICS.A  $OUTPUT_FOLDER/demo_L2_out.$corpus.$NUM_TOPICS.topwords $OUTPUT_FOLDER/report $NUM_COMP $NUM_ITRN $corpus $GIBBS_ITRN $MODEL $BETA_SMOOTH $SEED $NOISE_PROP 	
	fi
	...
	echo "Likelihood Computation for data"
    perl ./eval_EM.pl $OUTPUT_FOLDER/alpha_$corpus\_$NUM_TOPICS.txt $OUTPUT_FOLDER/pi_$corpus\_$NUM_TOPICS.txt $NUM_COMP $NUM_ITRN $corpus $OUTPUT_FOLDER $NUM_TOPICS $OUTPUT_FOLDER/$corpus\_train.in.mallet $OUTPUT_FOLDER/topics_$corpus\_$NUM_TOPICS.txt 
	echo "Likelihood Computation for held-out"
    perl ./eval_EM.pl $OUTPUT_FOLDER/alpha_$corpus\_$NUM_TOPICS.txt $OUTPUT_FOLDER/pi_$corpus\_$NUM_TOPICS.txt $NUM_COMP $NUM_ITRN $corpus $OUTPUT_FOLDER $NUM_TOPICS $OUTPUT_FOLDER/$corpus\_test.in.mallet $OUTPUT_FOLDER/topics_$corpus\_$NUM_TOPICS.txt
