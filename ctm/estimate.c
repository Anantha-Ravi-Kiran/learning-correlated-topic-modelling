...
llna_model* em_initial_model(int k, corpus* corpus, char* start, char* beta_input)
{
    llna_model* model;
    ...
    if (strcmp(start, "rand")==0)
        ...
        model = mom_init(k, corpus->nterms, beta_input);
    else
...
//Modified : Ravi
void em(char* dataset, int k, char* start, char* dir, char* beta_input)
{
    FILE* lhood_fptr;
    char string[100];
    int iteration;
    ...
    model = em_initial_model(k, corpus, start, beta_input);
...
int main(int argc, char* argv[])
{
    if (argc > 1)
    {
        if (strcmp(argv[1], "est")==0)
        {
			...
			if(argc > 7)	
				// MoM initialization
				em(argv[2], atoi(argv[3]), argv[4], argv[5], argv[7]);
			else
				em(argv[2], atoi(argv[3]), argv[4], argv[5], "NIL");
